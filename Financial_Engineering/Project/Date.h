...
public:
    ...
    datetime_t(int y, int m, int d, int hour_ = 0, int min_ = 0, int sec_ = 0, int ms_ = 0) : year(y), month(m), day(d), hour(hour_), min(min_), sec(sec_), ms(ms_) {};
    datetime_t(){}
    datetime_t(uint64_t unix_epoch_sec) {
//        const time_t second = unix_epoch_sec;
        time_t second = unix_epoch_sec;
        second = second/1000;   //removed ms
...
//        std::cout << "UTC time: " << year << "-" << month << "-" << day << " " << hour << ":" << min << ":" << sec << std::endl;
    }
