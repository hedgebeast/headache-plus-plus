...
int main(int argc, char** argv) {
    ...
    //[&] captures variables by reference, allowing the lambda to access and modify them directly
    auto feeder_listener = [&] (const Msg& msg) {       
        if (msg.isSet) {
            std::cout << "Timestamp: " << msg.timestamp << ", isSnap = " << msg.isSnap << ", numUpdates = " << msg.Updates.size() << std::endl << std::endl;
        }
    };
