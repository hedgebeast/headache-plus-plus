...
//relies on the L3Book::ForEachLevel to iterate the levels on both sides simultaneously, from inner levels to outer levels
...
//        printf("%10zu %10zu %10.2f %10.2f %10zu %10zu\n", bid_count, bid_qty, bid_price, ask_price, ask_qty, ask_count);
        printf("%10lld %10lld %10.2f %10.2f %10lld %10lld\n", bid_count, bid_qty, bid_price, ask_price, ask_qty, ask_count);
        return true;
    });
}


//relies on the L3Book::ForEachOrder to firstly iterate the sell side from outer levels to inner levels 
//and then buy side from inner levels to outer levels. For each price level, it iterates the price level 
//following the priority of the order, i.e. the FIFO ordering
...
//            printf("\n%5s %10.2f %10zu %4s", is_buy ? "B" : "S", level_price, level_qty, "");
            printf("\n%5s %10.2f %10lld %4s", is_buy ? "B" : "S", level_price, level_qty, "");
        }
//        printf("%zu(%zu), ", qty, order_id);
        printf("%lld(%lld), ", qty, order_id);
