...
void L3Book::UncrossBookSide(bool is_buy) {
    // TODO 
    //When a order is added to the buy side with price P, any sell orders with prices less or equal to P are stale orders and should be purged. 
    //Similarly when a order is added to the sell side with price P, any buy orders with prices greater or equal to P are considered to be stale orders.
    ...
    if (is_buy){
            ...
            for (auto innerItr = sell_side_.rbegin(); innerItr != sell_side_.rend(); innerItr++ ){   
                ...
                if (innerBid <= P) {
                    for(auto& order : innerLevel.orders) {
                        stale_orders.push_back(order->order_id);    //remove any buy orders which could cross with the sell side
                    }
                }
            }
        } 
        ...
        for (auto& sellItr : sell_side_){
            ...
            for (auto innerItr = buy_side_.begin(); innerItr != buy_side_.end(); innerItr++ ){   
                ...
                if (innerAsk >= P){
                    for (auto& order : innerLevel.orders) {
                        stale_orders.push_back(order->order_id);    //remove any sell orders which could cross with the buy side
                    }
                }
            }
        }
...
void L3Book::ProcessExec(int64_t order_id, int64_t exec_qty) {
    ...
    //partial execution: the exec_qty is less than the open_qty
    if (exec_qty < order_qty) 
        ProcessReplace(order_id, order.is_buy, order.price, (order_qty - exec_qty));
    
    //full execution: the exec_qty is equal to the open_qty
    else if (exec_qty == order_qty) 
        ProcessDelete(order_id);
    
    else
        throw std::runtime_error("order not present or exceeded execution amount :(");
}
